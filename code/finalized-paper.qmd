...
plant_subset %>% 
  ...
  ggpairs()
```
**Figure 3. Pairs Plot.** Variables of interest are listed on both x and y axes. 
...
Based on the tests run, we found that the full model including species, feedlevel, sla, chlorophyll, amass, num_lvs, and num_phylls predictors best predicted total biomass of pitcher plants (F = 38.38 on 15 and 87 DF, R ^ 2 = 0.85, p < 0.001, n = 103, α = 0.05). This model was chosen as it had the lowest AIC value, which means the full model was the simplest model that explained the most variance. 
...
table <- tidy(full_log, conf.int = TRUE) %>% 
    ...
    # change the estimates, stan error, and t-statistics to round to 2 digits
  # using mutate
...
In addition, what the full model does it that it takes all the predictors and fits them into a model to determine the predicted biomass of the pitcher plants. Below is a visualization of model predictions for biomass as function of chlorophyll. 


```{r}
plot(ggpredict(full_log, terms = "chlorophyll", back.transform = TRUE), add.data = TRUE)
```
**Figure 4. Model Prediction for biomass as a function of chlorophyll.** Black line represents predicted biomass of pitcher plants based on chlorophyll. Shaded gray area represents 95% confidence interval for predicted biomass. Jittered points are original data. 
...
```{r}
model_pred <- ggpredict(full_log, terms = "chlorophyll", back.transform = TRUE)
